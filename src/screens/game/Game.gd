...
func _draw() -> void:
	...
	Painter.drawSquareAtPosition(
		self,
		playfieldOrigin + Vector2(2 * SW, 2 * SW),
		Colors.TETROMINO_GREEN,
	)
	Painter.drawSquareAtPosition(
		self,
		playfieldOrigin + Vector2(2 * SW, 3 * SW),
		Colors.TETROMINO_PURPLE,
	)
	Painter.drawSquareAtPosition(
		self,
		playfieldOrigin + Vector2(3 * SW, 2 * SW),
		Colors.TETROMINO_RED,
	)
	Painter.drawSquareAtPosition(
		self,
		playfieldOrigin + Vector2(3 * SW, 1 * SW),
